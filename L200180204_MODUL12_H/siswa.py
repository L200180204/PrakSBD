...
class siswa:
    def __init__(self, master):
        self.master = master
        self.master.title("Database siswa")
        self.master.geometry('300x300')
        self.frame = Frame(self.master)
        self.frame.grid()

        title = Label(self.frame, text="Database Siswa", font=('Times', 16, 'bold'))
        l_nama = Label(self.frame, text="Nama", font=('Times', 12))
        l_nis = Label(self.frame, text="NIS", font=('Times', 12))
    ...
    def insert_member(self):
        c = db.cursor()
        sql =f"INSERT INTO member (`nama`,`niS`, `alamat`)VALUES('{self.e_nama.get()}','{self.e_nis.get()}', '{self.e_alamat.get()}')"         
        c.execute(sql)
        db.commit()
    ...
    def update_member(self):
        ...
        sql =f"UPDATE siswa SET nama=%s, nis=%s WHERE alamat=%s"
        val = (e1,e2,e3,e4)
        c.execute(sql,val)
        db.commit()
